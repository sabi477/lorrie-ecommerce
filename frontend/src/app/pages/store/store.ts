...
function mapProduct(p: Product, index: number): StoreProduct {
  ...
  const addDiscount = index % 3 === 0 && price > 1;
  ...
  return {
    ...
    brand:         'Lorrie',
    price,
    originalPrice: addDiscount ? Math.round(price * 1.25 * 100) / 100 : undefined,
    category:      cat,
    badge,
    rating:        3.8 + (index % 12) * 0.1,
    reviews:       50 + (index * 17) % 5000,
    bg:            palette.bg,
...
export class Store implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.productService.getAll(60).subscribe({
      next: (products) => {
