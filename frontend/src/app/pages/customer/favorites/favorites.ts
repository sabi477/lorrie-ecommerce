...
export class CustomerFavorites {
  ...
  readonly categories = computed(() => {
    const cats = [...new Set(this.favSvc.items().map(i => i.category))];
    return ['Tümü', ...cats];
  });

  readonly filtered = computed(() => {
    const f = this.activeFilter();
    return f === 'Tümü' ? this.favSvc.items() : this.favSvc.items().filter(i => i.category === f);
  });
  ...
  stars(r: number) { return Array(5).fill(0).map((_, i) => i < Math.floor(r)); }
}
